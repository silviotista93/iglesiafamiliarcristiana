...
class Cmsmasters_Donations_PayPal extends Cmsmasters_Donations_Gateway {
	...
	public function __construct() {
		...
		$this->gateway_name = esc_attr__('PayPal Checkout', 'cmsmasters_donations');
		...
		$this->settings = array( 
			array( 
				...
				'std' => 		'', 
				'label' => 		__('PayPal Email', 'cmsmasters_donations'), 
				'desc' => 		__('Your PayPal email address.', 'cmsmasters_donations'), 
				'type' => 		'input', 
			...
			array( 
				...
				'std' => 		'', 
				'label' => 		__('PayPal Identity Token', 'cmsmasters_donations'), 
				'desc' => 		__("Optionally enable 'Payment Data Transfer' (Profile > Website Payment Preferences) and then copy your identity token here. <br />This will allow payments to be verified without the need for PayPal IPN.", 'cmsmasters_donations'), 
				'type' => 		'input', 
				'class' => 		'gateway-settings gateway-settings-paypal' 
			), 
			array( 
				...
				'std' => 		'no', 
				'label' => 		__('PayPal Sandbox', 'cmsmasters_donations'), 
				'desc' => 		__('Enable PayPal Sandbox (use for testing)', 'cmsmasters_donations'), 
				'options' => array( 
	...
	public function paypal_recurring_args($paypal_args) {
		...
		$paypal_args['item_name'] = esc_attr__('New recurring donation', 'cmsmasters_donations') . ' &quot;' . get_the_title($donation_id) . '&quot;';
    ...
    public function return_handler() {
	    ...
	    if (!empty($posted['cm'])) {
			...
			case 'completed':
				...
				$this->send_admin_email($donation_id, sprintf(esc_html__('Payment has been received in full for donation #%d - this donation has been published.', 'cmsmasters_donations'), $donation_id));
    ...
    public function valid_paypal_ipn_request() {
	    ...
	    if (!empty($posted['custom'])) {
			...
			case 'completed':
				...
				break;
			case 'pending':
				$this->send_admin_email($donation_id, sprintf(esc_html__('PayPal payment is pending for donation #%d - this donation has *not* been automatically approved.', 'cmsmasters_donations'), $donation_id));
				
				
				break;
			case 'chargeback':
				$this->send_admin_email($donation_id, sprintf(esc_html__('The payment for donation #%d was reversed. Please check this donation/payment and change the status manually if need be.', 'cmsmasters_donations'), $donation_id));
