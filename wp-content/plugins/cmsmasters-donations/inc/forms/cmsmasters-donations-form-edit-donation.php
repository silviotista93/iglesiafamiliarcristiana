...
class Cmsmasters_Donations_Form_Edit_Donation extends Cmsmasters_Donations_Form_Submit_Donation {
	...
	public static function submit() {
		...
		if ( 
			...
			echo wpautop(__('Invalid donation', 'cmsmasters_donations'));
		...
		get_cmsmasters_donations_template('donation-submit.php', array( 
			...
			'submit_button_text' => 	__('Update donation', 'cmsmasters_donations') 
		) );
	}
	...
	public static function submit_handler() {
		...
		try {
			...
			echo '<div class="cmsmasters-donations-message">' . 
				__('Your changes have been saved.', 'cmsmasters_donations') . 
				' <a href="' . get_permalink(self::$donation_id) . '">' . __('View donation', 'cmsmasters_donations') . '</a>' . 
			'</div>';
		} catch (Exception $e) {
