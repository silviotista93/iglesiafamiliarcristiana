...
class Cmsmasters_Donations_Form_Submit_Donation extends Cmsmasters_Donations_Form {
		...
		self::$fields = array(
			'donation' => array(
				'donation_amount' => array(
					'label' => 			__('Monto de Donación', 'cmsmasters_donations'),
					'type' => 			'donation_amount',
					'required' => 		true,
					'options' => 		explode(',', str_replace(' ', '', get_option('cmsmasters_payment_amount', '5,10,20,50,100'))),
					'placeholder' => 	__('o ingrese su propio monto de donación, e.g: 85', 'cmsmasters_donations'),
					'validation' => 	'required, custom[number]',
					'priority' => 		1
				),
				'donation_payment_method' => array(
					'label' => 			__('Método de Pago', 'cmsmasters_donations'),
					'description' => 	__('Selecciona el método de pago', 'cmsmasters_donations'),
					'type' => 			'radio',
					'required' => 		true,
					'value' => 			'online',
					'options' => array(
						'online' => 	__('Pago en línea', 'cmsmasters_donations'),
						'offline' => 	__('Pago fuera de línea', 'cmsmasters_donations')
					),
					'validation' => 	'required',
					'priority' => 		2
				),
				'recurring_donation' => array(
					'label' => 			__('Recurrente', 'cmsmasters_donations'),
					'description' => 	__('Seleccione el intervalo de pago', 'cmsmasters_donations'),
					'type' => 			'select',
					'required' => 		true,
					'value' => 			'',
					'options' => array(
						'1' => 		__('Una vez', 'cmsmasters_donations'),
						'7' => 		__('Semanal', 'cmsmasters_donations'),
						'30' => 	__('Mensual', 'cmsmasters_donations'),
						'365' => 	__('Anual', 'cmsmasters_donations')
					),
					'validation' => 	'',
					'priority' => 		3
				),
				'donation_campaign' => array(
					'label' => 			__('Motivo', 'cmsmasters_donations'),
					'description' => 	__('Seleccione el motivo a la que le gustaría contribuir', 'cmsmasters_donations'),
					'type' => 			'select',
				...
				'donation_message' => array(
					'label' => 			__('Mensaje', 'cmsmasters_donations'),
					'type' => 			'textarea',
					'required' => 		((get_option('cmsmasters_donator_message') == 'required') ? true : false),
					'placeholder' => 	__('Escribe tu mensaje...', 'cmsmasters_donations'),
					'validation' => 	((get_option('cmsmasters_donator_message') == 'required') ? 'required' : ''),
					'priority' => 		5
				),
				'anonymous_donation' => array(
					'label' => 			__('¿Donacion anonima?', 'cmsmasters_donations'),
					'type' => 			'checkbox',
					'required' => 		false,
					'description' => 	__('Marque esta casilla para ocultar su información personal en nuestra lista de donantes', 'cmsmasters_donations'),
					'value' => 			'false',
					'validation' => 	'',
					'priority' => 		6
				)
			...
			'donator' => array(
				'donator_firstname' => array(
					'label' => 			__('Nombre', 'cmsmasters_donations'),
					'type' => 			'text',
				...
				'donator_lastname' => array(
					'label' => 			__('Apellido', 'cmsmasters_donations'),
					'type' => 			'text',
				...
				'donator_address' => array(
					'label' => 			__('Dirección', 'cmsmasters_donations'),
					'type' => 			'text',
				...
				'donator_city' => array(
					'label' => 			__('Ciudad', 'cmsmasters_donations'),
					'type' => 			'text',
				...
				'donator_state' => array(
					'label' => 			__('Departamento', 'cmsmasters_donations'),
					'type' => 			'text',
				...
				'donator_zip' => array(
					'label' => 			__('Código Postal', 'cmsmasters_donations'),
					'type' => 			'text',
				...
				'donator_country' => array(
					'label' => 			__('Pais', 'cmsmasters_donations'),
					'type' => 			'text',
				...
				'donator_phone' => array(
					'label' => 			__('Teléfono', 'cmsmasters_donations'),
					'type' => 			'text',
					'required' => 		((get_option('cmsmasters_donator_phone') == 'required') ? true : false),
					'placeholder' => 	__('', 'cmsmasters_donations'),
				...
				'donator_website' => array(
					'label' => 			__('Sitio web', 'cmsmasters_donations'),
					'type' => 			'text',
					'required' => 		((get_option('cmsmasters_donator_website') == 'required') ? true : false),
					'placeholder' => 	__('http://', 'cmsmasters_donations'),
	...
	public static function submit() {
		...
		get_cmsmasters_donations_template('donation-submit.php', array(
			...
			'submit_button_text' => 	((get_option('cmsmasters_confirm_donation') == 1) ? __('Confirmar Donación', 'cmsmasters_donations') : __('Enviar Donación', 'cmsmasters_donations'))
		) );
	...
	public static function preview() {
		...
		if (self::$donation_id) {
			...
			echo "<form method=\"post\" id=\"donation_preview\" data-method=\"" . get_post_meta(self::$donation_id, 'cmsmasters_donation_payment_method', true) . "\">
				<input type=\"submit\" name=\"edit_donation\" class=\"button\" value=\"" . __('Editar Donación', 'cmsmasters_donations') . "\" />
				<input type=\"submit\" name=\"continue\" id=\"donation_preview_submit_button\" class=\"button\" value=\"" . apply_filters('submit_donation_step_preview_submit_text', __('Enviar Donación', 'cmsmasters_donations')) . "\" />
	...
	protected static function save_donation($post_content = '', $status = 'preview') {
		$donation_data = array(
			'post_title' => 		__('Nueva Donación', 'cmsmasters_donations'),
		...
		$donation_title = array(
			'ID' => 			self::$donation_id,
			'post_title' => 	__('Donación #', 'cmsmasters_donations') . self::$donation_id
		);
		...
		foreach (self::$fields as $group_key => $fields) {
			foreach ($fields as $key => $field) {
				if (
					...
					return new WP_Error('validation-error', sprintf(__("%s debe ser un número válido. No incluir símbolo de moneda.", 'cmsmasters_donations'), $field['label']));
				}
				...
				if (
					...
					return new WP_Error('validation-error', sprintf(__('%s es un campo obligatorio', 'cmsmasters_donations'), $field['label']));
				}
			}
		}
	...
	private static function donation_campaigns() {
		if (get_option('cmsmasters_payment_campaign') == 'required') {
			...
			$options = array(
				'' => __('No hay un motivo específico.', 'cmsmasters_donations')
			);
		}
		...
		foreach ($campaigns as $key => $value) {
			$options[$key] = sprintf(__('%s - (Target: %s)', 'cmsmasters_donations'), $value, get_the_campaign_target($key));
		}
