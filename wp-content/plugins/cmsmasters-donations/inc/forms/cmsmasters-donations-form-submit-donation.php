...
class Cmsmasters_Donations_Form_Submit_Donation extends Cmsmasters_Donations_Form {
		...
		self::$fields = array( 
			'donation' => array( 
				'donation_amount' => array( 
					...
					'placeholder' => 	__('or enter your own donation amount, e.g: 85', 'cmsmasters_donations'), 
					'validation' => 	'required, custom[number]', 
					'priority' => 		1 
				), 
				'donation_payment_method' => array( 
					'label' => 			__('Payment Method', 'cmsmasters_donations'), 
					'description' => 	__('Select the payment method', 'cmsmasters_donations'),
					'type' => 			'radio',  
					'required' => 		true, 
					'value' => 			'online', 
					'options' => array( 
						'online' => 	__('Online payment', 'cmsmasters_donations'), 
						'offline' => 	__('Offline payment', 'cmsmasters_donations') 
					), 
					'validation' => 	'required', 
					'priority' => 		2 
				), 
				'recurring_donation' => array( 
					'label' => 			__('Recurrence', 'cmsmasters_donations'), 
					'description' => 	__('Select the payment interval', 'cmsmasters_donations'), 
					'type' => 			'select', 
					'required' => 		true, 
					'value' => 			'', 
					'options' => array( 
						'1' => 		__('One-time', 'cmsmasters_donations'), 
						'7' => 		__('Weekly', 'cmsmasters_donations'), 
						'30' => 	__('Monthly', 'cmsmasters_donations'), 
						'365' => 	__('Yearly', 'cmsmasters_donations') 
					), 
					'validation' => 	'', 
					'priority' => 		3 
				), 
				'donation_campaign' => array( 
					'label' => 			__('Campaigns', 'cmsmasters_donations'), 
					'description' => 	__('Select the campaign you would like to contribute to', 'cmsmasters_donations'), 
					'type' => 			'select', 
				...
				'donation_message' => array( 
					...
					'placeholder' => 	__('Your custom message text...', 'cmsmasters_donations'), 
				...
				'anonymous_donation' => array( 
					...
					'type' => 			'checkbox', 
					'required' => 		false, 
					'description' => 	__('Check this box to hide your personal info in our donators list', 'cmsmasters_donations'), 
					'value' => 			'false', 
					'validation' => 	'', 
					'priority' => 		6 
				) 
			...
			'donator' => array( 
				...
				'donator_phone' => array( 
					...
					'placeholder' => 	__('area code + phone number', 'cmsmasters_donations'), 
	...
	public static function submit() {
		...
		get_cmsmasters_donations_template('donation-submit.php', array( 
			...
			'submit_button_text' => 	((get_option('cmsmasters_confirm_donation') == 1) ? __('Confirm donation', 'cmsmasters_donations') : __('Submit donation', 'cmsmasters_donations')) 
		) );
	...
	public static function preview() {
		...
		if (self::$donation_id) {
			...
			echo "<form method=\"post\" id=\"donation_preview\" data-method=\"" . get_post_meta(self::$donation_id, 'cmsmasters_donation_payment_method', true) . "\">
				<input type=\"submit\" name=\"edit_donation\" class=\"button\" value=\"" . __('Edit donation', 'cmsmasters_donations') . "\" />
				<input type=\"submit\" name=\"continue\" id=\"donation_preview_submit_button\" class=\"button\" value=\"" . apply_filters('submit_donation_step_preview_submit_text', __('Submit Donation', 'cmsmasters_donations')) . "\" />
	...
	protected static function save_donation($post_content = '', $status = 'preview') {
		$donation_data = array( 
			'post_title' => 		__('New Donation', 'cmsmasters_donations'), 
		...
		$donation_title = array( 
			'ID' => 			self::$donation_id, 
			'post_title' => 	__('Donation #', 'cmsmasters_donations') . self::$donation_id 
		);
		...
		foreach (self::$fields as $group_key => $fields) {
			foreach ($fields as $key => $field) {
				if ( 
					...
					return new WP_Error('validation-error', sprintf(__("%s must be a valid number. Don't include currency symbol.", 'cmsmasters_donations'), $field['label']));
				}
				...
				if ( 
					...
					return new WP_Error('validation-error', sprintf(__('%s is a required field', 'cmsmasters_donations'), $field['label']));
				}
			}
		}
	...
	private static function donation_campaigns() {
		if (get_option('cmsmasters_payment_campaign') == 'required') {
			...
			$options = array( 
				'' => __('No specific campaign', 'cmsmasters_donations') 
			);
		}
