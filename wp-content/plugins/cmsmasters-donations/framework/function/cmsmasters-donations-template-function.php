...
function get_the_donation_status($post = null) {
	...
	if ($status == 'publish') {
		$out = esc_attr__('Validated', 'cmsmasters_donations');
	} elseif ($status == 'pending_payment') {
		$out = esc_attr__('Pending Payment', 'cmsmasters_donations');
	} elseif ($status == 'pending_offline') {
		$out = esc_attr__('Pending Offline Payment', 'cmsmasters_donations');
	} else {
		$out = esc_attr__('Pending', 'cmsmasters_donations');
	}
...
function get_the_recurrence_period($post = null) {
	...
	case '7':
		$period = esc_attr__('Weekly', 'cmsmasters_donations');
		
		
		break;
	case '30':
		$period = esc_attr__('Monthly', 'cmsmasters_donations');
		
		
		break;
	case '365':
		$period = esc_attr__('Yearly', 'cmsmasters_donations');
...
function get_the_campaign_target($campaign_id, $round = false) {
    ...
    if ($campaign_target != '') {
		...
		$campaign_target = esc_attr__('None', 'cmsmasters_donations');
    }
