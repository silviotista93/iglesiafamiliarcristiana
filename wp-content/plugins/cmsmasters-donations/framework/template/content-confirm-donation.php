...
echo '<div class="donation_confirm">' . 
	'<h2 class="donation_confirm_title">' . __('Confirm your', 'cmsmasters_donations') . ' ' . get_the_title() . '</h2>' . 
	
	'<div class="donation_confirm_info">' . 
		
		'<div class="donation_confirm_info_item donation_confirm_amount">' . 
			'<span class="donation_confirm_info_title">' . esc_html__('Amount', 'cmsmasters_donations') . '</span>' . 
		...
		if (get_the_recurrence_period($post) != '') {
			echo '<div class="donation_confirm_info_item donation_confirm_recurrence">' . 
				'<span class="donation_confirm_info_title">' . esc_html__('Recurrence', 'cmsmasters_donations') . '</span>' . 
		...
		if (get_the_donation_campaign()) {
			echo '<div class="donation_confirm_info_item donation_confirm_campaign">' . 
				'<span class="donation_confirm_info_title">' . esc_html__('Campaign', 'cmsmasters_donations') . '</span>' . 
		...
		echo '<div class="donation_confirm_info_item donation_confirm_received">' . 
			'<span class="donation_confirm_info_title">' . esc_html__('Received', 'cmsmasters_donations') . '</span>' . 
			'<span class="donation_confirm_info_value"><date>' . human_time_diff(get_the_time('U'), current_time('timestamp')) . ' ' . esc_html__('ago', 'cmsmasters_donations') . '</date></span>' . 
		'</div>' . 
		
		'<div class="donation_confirm_info_item donation_confirm_donator">' . 
			'<span class="donation_confirm_info_title">' . esc_html__('Donator', 'cmsmasters_donations') . '</span>' . 
	...
	if (is_anonymous_donation($post)) {
		echo '<p class="donation_confirm_anonymous">' . esc_attr__('This donation is anonymous', 'cmsmasters_donations') . '</p>';
	}
	
	
	if (get_the_excerpt() != '') {
		echo '<h4 class="donation_confirm_cont_title">' . apply_filters('the_donation_message_title', esc_attr__("Donator message", 'cmsmasters_donations')) . '</h4>';
