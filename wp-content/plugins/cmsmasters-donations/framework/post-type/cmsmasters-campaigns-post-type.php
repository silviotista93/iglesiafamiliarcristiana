...
class Cmsmasters_Campaigns_Post_Type {
	public function __construct() {
		...
		$campaign_labels = array( 
			'name' => 					__('Campaigns', 'cmsmasters_donations'), 
			'singular_name' => 			__('Campaign', 'cmsmasters_donations'), 
			'menu_name' => 				__('Campaigns', 'cmsmasters_donations'), 
			'all_items' => 				__('All Campaigns', 'cmsmasters_donations'), 
			'add_new' => 				__('Add New', 'cmsmasters_donations'), 
			'add_new_item' => 			__('Add New Campaign', 'cmsmasters_donations'), 
			'edit_item' => 				__('Edit Campaign', 'cmsmasters_donations'), 
			'new_item' => 				__('New Campaign', 'cmsmasters_donations'), 
			'view_item' => 				__('View Campaign', 'cmsmasters_donations'), 
			'search_items' => 			__('Search Campaigns', 'cmsmasters_donations'), 
			'not_found' => 				__('No campaigns found', 'cmsmasters_donations'), 
			'not_found_in_trash' => 	__('No campaigns found in Trash', 'cmsmasters_donations') 
		);
		...
		register_taxonomy('cp-categs', array('campaign'), array( 
			'hierarchical' => 		true, 
			'label' => 				__('Campaign Categories', 'cmsmasters_donations'), 
			'singular_label' => 	__('Campaign Category', 'cmsmasters_donations'), 
			'rewrite' => array( 
				'slug' => 			'cp-categs', 
				'with_front' => 	true 
			) 
		...
		register_taxonomy('cp-tags', array('campaign'), array( 
			'hierarchical' => 		false, 
			'label' => 				__('Campaign Tags', 'cmsmasters_donations'), 
			'singular_label' => 	__('Campaign Tag', 'cmsmasters_donations'), 
			'rewrite' => array( 
				'slug' => 			'cp-tags', 
				'with_front' => 	true 
			) 
	...
	public function edit_columns($columns) {
		...
		$new_columns = array(
			'cp_thumb' => 		__('Thumbnail', 'cmsmasters_donations'), 
			'cp_target' => 		__('Target', 'cmsmasters_donations'), 
			'cp_funds' => 		__('Funds', 'cmsmasters_donations'), 
			'cp_categs' => 		__('Categories', 'cmsmasters_donations'), 
			'date' => 			__('Date', 'cmsmasters_donations'), 
			'comments' => 		'<span class="vers"><div title="' . __('Comments', 'cmsmasters_donations') . '" class="comment-grey-bubble"></div></span>', 
			'menu_order' => 	'<span class="vers"><div class="dashicons dashicons-sort" title="' . __('Order', 'cmsmasters_donations') . '"></div></span>' 
		);
		...
		switch ($column) {
			case 'cp_thumb':
				if (has_post_thumbnail() != '') {
					...
					echo '<em>' . __('No Thumbnail', 'cmsmasters_donations') . '</em>';
				}
			...
			case 'cp_funds':
				...
				'<br />' . 
				'<span>' . __('count', 'cmsmasters_donations') . ' - <strong>' . $funds_number . '</strong></span>' . 
				'<br />' . 
				'<span>' . cmsmasters_donations_currency(round(($target - $funds), 2)) . ' ' . __('to go', 'cmsmasters_donations') . '</span>';
			...
			case 'cp_categs':
				if (get_the_terms(0, 'cp-categs') != '') {
					...
					echo '<em>' . __('Uncategorized', 'cmsmasters_donations') . '</em>';
				}
