...
if( ! class_exists( 'WPGP_Metabox' ) ) {
  class WPGP_Metabox extends WPGP_Abstract{
    ...
    public function add_meta_box_content( $post, $callback ) {
      ...
      $errors   = ( is_object ( $post ) ) ? get_post_meta( $post->ID, '_wpgp_errors', true ) : array();
      $errors   = ( ! empty( $errors ) ) ? $errors : array();
      $theme    = ( $this->args['theme'] ) ? ' wpgp-theme-'. $this->args['theme'] : '';

      if( is_object ( $post ) && ! empty( $errors ) ) {
        delete_post_meta( $post->ID, '_wpgp_errors' );
      }

      wp_nonce_field( 'wpgp_metabox_nonce', 'wpgp_metabox_nonce'. $this->unique );

      echo '<div class="wpgp wpgp-metabox'. $theme .'">';
    ...
    public function save_meta_box( $post_id ) {
      ...
      if( ! wp_verify_nonce( wpgp_get_var( 'wpgp_metabox_nonce'. $this->unique ), 'wpgp_metabox_nonce' ) ) {
        return $post_id;
      }
      ...
      $request = apply_filters( "wpgp_{$this->unique}_save", $request, $post_id, $this );

      do_action( "wpgp_{$this->unique}_save_before", $request, $post_id, $this );
      ...
      if( empty( $request ) || ! empty( $request['_restore'] ) ) {
        ...
        if( ! empty( $errors ) ) {
          update_post_meta( $post_id, '_wpgp_errors', $errors );
        }

      }

      do_action( "wpgp_{$this->unique}_saved", $request, $post_id, $this );

      do_action( "wpgp_{$this->unique}_save_after", $request, $post_id, $this );
