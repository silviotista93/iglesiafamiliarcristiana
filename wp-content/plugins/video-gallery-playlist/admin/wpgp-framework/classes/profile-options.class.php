 ...
 *
 */
if( ! class_exists( 'WPGP_Profile_Options' ) ) {
  class WPGP_Profile_Options extends WPGP_Abstract{
    ...
    public function render_profile_form_fields( $profileuser ) {
      ...
      $errors     = ( ! empty( $profile_id ) ) ? get_user_meta( $profile_id, '_wpgp_errors', true ) : array();
      $errors     = ( ! empty( $errors ) ) ? $errors : array();
      $class      = ( $this->args['class'] ) ? ' '. $this->args['class'] : '';

      // clear errors
      if( ! empty( $errors ) ) {
        delete_user_meta( $profile_id, '_wpgp_errors' );
      }

      echo '<div class="wpgp wpgp-profile wpgp-onload'. $class .'">';

      wp_nonce_field( 'wpgp_profile_nonce', 'wpgp_profile_nonce'. $this->unique );
      ...
      foreach( $this->sections as $section ) {
        ...
        if( ! empty( $section['fields'] ) ) {
          foreach( $section['fields'] as $field ) {
            ...
            WPGP::field( $field, $this->get_meta_value( $profile_id, $field ), $this->unique, 'profile' );
    ...
    public function save_profile( $user_id ) {
      ...
      if ( wp_verify_nonce( wpgp_get_var( 'wpgp_profile_nonce'. $this->unique ), 'wpgp_profile_nonce' ) ) {
        ...
        foreach ( $this->sections as $section ) {
          ...
          $request = apply_filters( "wpgp_{$this->unique}_save", $request, $user_id, $this );

          do_action( "wpgp_{$this->unique}_save_before", $request, $user_id, $this );
          ...
          if( empty( $request ) ) {
            ...
            if( ! empty( $errors ) ) {
              update_user_meta( $user_id, '_wpgp_errors', $errors );
            }

          }

          do_action( "wpgp_{$this->unique}_saved", $request, $user_id, $this );

          do_action( "wpgp_{$this->unique}_save_after", $request, $user_id, $this );
