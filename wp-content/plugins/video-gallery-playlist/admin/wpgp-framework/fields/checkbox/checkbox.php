 ...
 *
 */
if( ! class_exists( 'WPGP_Field_checkbox' ) ) {
  class WPGP_Field_checkbox extends WPGP_Fields {
    ...
    public function render() {
      ...
      if( isset( $this->field['options'] ) ) {
        ...
        if( is_array( $options ) && ! empty( $options ) ) {
          ...
          echo ( ! empty( $this->field['empty_message'] ) ) ? $this->field['empty_message'] : esc_html__( 'No data provided for this option type.', 'wpgp' );

        }

      } else {

        echo '<label class="wpgp-checkbox">';
        echo '<input type="hidden" name="'. $this->field_name() .'" value="'. $this->value .'" class="wpgp--input"'. $this->field_attributes() .'/>';
        echo '<input type="checkbox" name="_pseudo" class="wpgp--checkbox"'. checked( $this->value, 1, false ) .'/>';
